...
function transportadorController($scope, $compile, DTOptionsBuilder, DTColumnBuilder, ModalService, $rootScope, SysMgmtData, TableCreate,Datatablessss,tableOptionsFactory,tableColumnsFactory,FiltersFactory,validationFactory) {
        var vm = this;
        vm.selected = {};
        vm.selectAll = false;
       ...
       var actionsHtml = function (data, type, full, meta) {
        ...
        Datatablessss.getTable('/pessoa/api/cliente/fetchPage', fnDataSRC, new qat.model.empresaInquiryRequest(0, true, null, null, null), this, rCallback, null, recompile, tableOptionsFactory.transportador(vm,createdRow,$scope,FiltersFactory.transportador()), tableColumnsFactory.pessoa(vm,titleHtml,actionsHtml));
...
(function() {
    angular.module('wdApp.apps.transportador.delete', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('TransportadorDeleteController', function($rootScope, $scope, fModels, SysMgmtData, fPessoa) {
            ...
            console.log($rootScope.transportador)
            $scope.saveTransportador = function() {
                fPessoa.fnDelete($scope.transportador, "site/api/transportador/update/", function(){console.log('ddda   aqui')});
            }
        });
})();
(function() {
    angular.module('wdApp.apps.transportador.view', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('TransportadorViewController', function($rootScope, $scope, fModels, SysMgmtData, fPessoa) {
            ...
            console.log($rootScope.transportador)
            $scope.saveTransportador = function() {
                fPessoa.fnOpenView($scope.transportador,"site/api/transportador/update/", function(){console.log('aqui')});
            }
        });
})();
