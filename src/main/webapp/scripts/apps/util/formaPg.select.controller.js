...
angular.module('wdApp.apps.select.formaPg', ['ngSanitize', 'ui.select'])
...
}).controller('FormaPgSelectControllers', formaPgSelectControllers);

    function formaPgSelectControllers($scope, $http, $timeout, $interval) {
        var vm = this;


        $scope.forms = [{ nome : 'form1' ,formaPg :{id : 0}}];
        $scope.createForm = function(type){

            $scope.forms.push({ nome : 'form1' + ($scope.forms.length + 1),formaPg :{id : type}});
...
qat.model.select.util("financeiro/api/formaPg/fetchPage",true,new qat.model.planoInquiryRequest( 100/20, true, JSON.parse(localStorage.getItem("empresa")).id),callbackBanco);
  //qat.model.select.anonimo("financeiro/api/formaPg/fetchPage",true,new qat.model.planoInquiryRequest( 100/20, true, null),callbackBanco);
