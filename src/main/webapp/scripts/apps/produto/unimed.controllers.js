(function() {
    ...
    function uniMedController($scope, fModels, $compile, DTOptionsBuilder, DTColumnBuilder, ModalService, $rootScope, SysMgmtData, Datatablessss, dialogFactory, $filter, toastr) {
        ...
        // remove user
        $scope.removeUser = function(uniMed) {
...
(function() {
    angular.module('wdApp.apps.uniMed.insert', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('UniMedInsertController', function($rootScope, $scope, fModels, SysMgmtData, fPessoa) {
            var vm = this;
            $scope.uniMed = {};
...
(function() {
    angular.module('wdApp.apps.uniMed.update', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('UniMedUpdateController', function($rootScope, $scope, fModels, SysMgmtData, fPessoa) {
            var vm = this;
            $scope.uniMed = {};
            $scope.uniMed = $rootScope.uniMed;
            console.log($rootScope.uniMed)
            $scope.saveUniMed = function() {
                fPessoa.fnMontaObjeto($scope.uniMed, $scope.endereco, 'UPDATE', "produto/api/uniMed/update/", fnCallBack);
            }
        });
})();
(function() {
    angular.module('wdApp.apps.uniMed.delete', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('UniMedDeleteController', function($rootScope, $scope, fModels, SysMgmtData, fPessoa) {
            var vm = this;
            $scope.uniMed = {};
            $scope.uniMed = $rootScope.uniMed;
            console.log($rootScope.uniMed)
            $scope.saveUniMed = function() {
                fPessoa.fnDelete($scope.uniMed, "produto/api/uniMed/update/", function() { console.log('ddda   aqui') });
            }
        });
})();
(function() {
    angular.module('wdApp.apps.uniMed.view', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('UniMedViewController', function($rootScope, $scope, fModels, SysMgmtData, fPessoa) {
            var vm = this;
            $scope.uniMed = {};
            $scope.uniMed = $rootScope.uniMed;
            console.log($rootScope.uniMed)
            $scope.saveUniMed = function() {
                fPessoa.fnOpenView($scope.uniMed, "produto/api/uniMed/update/", function() { console.log('aqui') });
            }
        });
})();
(function() {
    angular.module('wdApp.apps.uniMed.select', ['ngSanitize', 'ui.select'])
        ...
        }).controller('UniMedSelectController', uniMedSelectController);

    function uniMedSelectController($scope, $http, $timeout, $interval, dialogFactory, validationFactory) {
        var vm = this;
        ...
        var closse = function() {
            var callbackBanco = function(res) {
                ...
                if (res.operationSuccess == true) {
                    ...
                    vm.people = res.unimedList;

                }
            }

            // qat.model.select.anonimo("fiscal/api/cnae/fetchPage",true,new qat.model.planoInquiryRequest( 100/20, true, null),callbackBanco);
            qat.model.select.util("produto/api/unimed/fetchPage", true, new qat.model.planoInquiryRequest(100 / 20, true, JSON.parse(localStorage.getItem("empresa")).id), callbackBanco);

        }

        vm.addUniMed = function() {

            dialogFactory.dialog('views/produto/dialog/dUniMed.html', "UniMedInsertController", validationFactory.uniMed, closse);
