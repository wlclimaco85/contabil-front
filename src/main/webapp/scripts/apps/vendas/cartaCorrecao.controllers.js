(function() {
    angular.module('wdApp.apps.cartaCorrecao', ['datatables','ngResource', 'datatables.scroller', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('CartaCorrecaoController', AngularWayChangeDataCtrl).filter("myfilter", function() {
    ...
    var vm = this;
       ...
       var reloadData = function() {
        ...
        function fnEdit(person) {
            ...
            dialogFactory.dialog('views/financeiro/dialog/dCartaCorrecao.html',"CartaCorrecaoUpdateController",validationFactory.contasPagar(),reloadData());
        }

        function fnDelete(person) {
           $rootScope.doisValor = person;
           dialogFactory.dialog('views/util/dialog/dDelete.html',"CartaCorrecaoDeleteController",validationFactory.contasPagar(),reloadData());
        }
...
(function() {
    angular.module('wdApp.apps.cartaCorrecao.insert', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('CartaCorrecaoInsertController', function($rootScope, $scope, fModels, SysMgmtData,toastr,$element, close) {
            var vm = this;
            $scope.contasPagar = {};
            $scope.contasPagar = $rootScope.contasPagar;
            console.log($rootScope.contasPagar)
            $scope.saveDoisValor = function() {
                var oObject = qat.model.fnDoisValores(null, $scope.cartaCorrecao.value, $scope.cartaCorrecao.nome, "SITUACAO", "INSERT",$rootScope.user.user,$scope.cartaCorrecao.descricao,106,101)
...
(function() {
    angular.module('wdApp.apps.cartaCorrecao.update', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('CartaCorrecaoUpdateController', function($rootScope, $scope, fModels, SysMgmtData,toastr,$element, close) {
            var vm = this;

            console.log($rootScope.doisValor)
            $scope.cartaCorrecao = $rootScope.doisValor
            $scope.saveDoisValor = function() {
                var oObject = qat.model.fnDoisValores($scope.cartaCorrecao.id, $scope.cartaCorrecao.value, $scope.cartaCorrecao.nome, "SITUACAO", "UPDATE",$rootScope.user.user,$scope.cartaCorrecao.descricao,106,101)
...
(function() {
    angular.module('wdApp.apps.cartaCorrecao.delete', ['datatables', 'angularModalService', 'datatables.buttons', 'datatables.light-columnfilter'])
        .controller('CartaCorrecaoDeleteController', function($rootScope, $scope, fModels, SysMgmtData,toastr,$element, close) {
            var vm = this;
